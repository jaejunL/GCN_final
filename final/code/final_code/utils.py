...
# 
# Class AverageMeter, CosineLR, MelDataset, WaveDataset,
# Function compute_gain, and mix are from the github
# https://github.com/OsciiArt/Freesound-Audio-Tagging-2019
# 
# Function _one_sample_positive_class_precisions,
# and calculate_per_class_lwlrap are from the DCASE 2019 task 2
# http://dcase.community/challenge2019/task-audio-tagging
# 
# Function gen_A and gen_adj are from the github
...
class MelDataset_for_test(Dataset):
    def __init__(self, X, y, crop=-1,
                 mixup=False, freqmask=False, gain=False,
                 crop_mode='original',crop_rate=0.25
                 ):
...
def gen_A2(num_classes, adj_file):
    import pickle
    result = pickle.load(open(adj_file, 'rb'))
    _adj = result['adj']
    _adj = _adj * 0.25 / (_adj.sum(0, keepdims=True) + 1e-6)
