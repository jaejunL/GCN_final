...
sys.path.append('.')
from utils import AverageMeter, cycle, CosineLR, MelDataset, calculate_per_class_lwlrap, gen_A, gen_adj
from models import ResNet, GCNResNet
...
FOLD_LIST = [1, 2, 3, 4, 5]
CROP_LENGTH = 512
OUTPUT_DIR = "../../log/test/gcn2"

cudnn.benchmark = True
starttime = time.time()

df_train = pd.read_csv("../../data/FSDKaggle2019.meta/train_curated_post_competition.csv")
df_noisy = pd.read_csv("../../data/FSDKaggle2019.meta/train_noisy_post_competition.csv")
...
df_train['path'] = "../../data/dcase/mel128/train/" + df_train['fname']
df_noisy['path'] = "../../data/dcase/mel128/noisy/" + df_noisy['fname']
...
for fold, (ids_train_split, ids_valid_split) in enumerate(folds):
    ...
    for epoch in range(NUM_EPOCH):
        ...
        if val_lwlrap > min_val_lwlrap:
            ...
            trigger = 0
            torch.save(model.state_dict(), "{}/weight_fold_{}_best.pth".format(OUTPUT_DIR, fold+1, epoch+1))
